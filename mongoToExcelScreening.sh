...
mongoexport -u admin123 -p Jzfq2n6b4n15 --authenticationDatabase=admin --db=javix --collection=screeningcases --out=/mnt/volume_blr1_01/javix/Javix-BackEnd/uploads/documents/dailyScreeningScreener.csv -q='{"issubscreener" : 1}' --type=csv --fields=citizenId,screenerId,createdAt
#  -q="{ "${k}and": [ { "createdAt": { "${k}gte": { "${k}date": ${s}"$yesterday"${s} } } }, { "createdAt": { "${k}lte": { "${k}date": ${s}"$today"${s} } } } ] }"

mongoexport -u admin123 -p Jzfq2n6b4n15 --authenticationDatabase=admin --db=javix --collection=screeningcases  --out=/mnt/volume_blr1_01/javix/Javix-BackEnd/uploads/documents/weeklyScreeningScreener.csv -q='{"issubscreener" : 1 }' --type=csv --fields=citizenId,screenerId,createdAt
