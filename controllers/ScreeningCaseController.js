...
// 	   ScreeningCaseModel.ScreeningCase.count({  severity_bp: 2, severity_bmi: 2,'$lipidcritical.Age':{$gte:40}} , async (err, totalCount) => {
// 		 if (err) {
// 		   response = { error: true, message: 'Error fetching data' }
// 		 }
//      ScreeningCaseModel.ScreeningCase.find({}, {}, query, async (err, data) => {
...
// for count 
lipidcount = await  ScreeningCaseModel.ScreeningCase.aggregate([
      {$sort:{'createdAt':-1} },
      {
        $lookup: {
          localField: "citizenId",
      ...
      // // lipidcount= lipidcount[0].count
      // }
      // })
    
      lipidcountFinal = lipidcount[0].count;
      console.log(lipidcountFinal);
    //for pagination
	
    lipiddata = await  ScreeningCaseModel.ScreeningCase.aggregate([
          {$sort:{'createdAt':-1} },
          {
            $lookup: {
              localField: "citizenId",
      ...
      //   $match: {
      //     count: {
      //       Age: { $gte: 40 }, severity_bp: 2, severity_bmi: 2 
      //     }
      //   }
      // },
      // {
  ...
  // .exec((err, likeData,lipidcountFinal) => {
  ...
  //   // total: count,
  //   size: size,
  //   total:lipidcountFinal,
  //   likeData: likeData.reverse(),
