...
exports.register = [
	...
	body("userName").isLength({ min: 1 }).trim().withMessage("UserName must be specified.")
		.isAlphanumeric().withMessage("UserName must be a unique userName.").custom((value) => {
	...
	body("phoneNo1")
					.custom((value) => {
						...
						return Promise.reject("Phone no 1. should have 10 digits with no preceeding zero!");
					}
				}
	...
	body("roleId").isLength({ min: 1,max:99 }).trim().withMessage("Invalid Form Entry [Role Id]."),
	...
	// 		// Extract the validation errors from a request.
			...
			if (!errors.isEmpty()) {
				...
				bcrypt.hash(req.body.password,10,function(err, hash) {
					...
					var num;
					if(req.body.phoneNo!=undefined && req.body.phoneNo!="" && req.body.phoneNo!=null ){
						num='91'+req.body.phoneNo;
					}
					else if(req.body.phoneNo1!=undefined && req.body.phoneNo1!="" && req.body.phoneNo1!=null ){
...
exports.login = [
	...
	var options = { method: 'POST',
					  url: 'http://18.60.238.252:3010/api/login/getjavixid',
					  headers: 
