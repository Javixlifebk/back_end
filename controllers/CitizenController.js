    ...
    if (value != null && value != "" && value != undefined) {
      if (value.length != 10 || value[0] === "0" || isNaN(value)) {
        return Promise.reject(
          "Mobile no. should have 10 digits with no preceeding zero!"
        );
      }
    }
      ...
      if (!errors.isEmpty()) {
        ...
        bcrypt.hash(passwordGenrated, 10, function (err, hashPassword) {
          ...
          javixId =
            req.body.state +
            "/" +
            req.body.district.substring(0, 2) +
            "/" +
            ngoRowNo +
            "/" +
            req.body.firstName.substring(0, 1) +
            "" +
            req.body.lastName.substring(0, 1) +
            "" +
            dobEscape +
            "/" +
            randThree;
          ...
          actionCitizen.save(function (_error) {
            if (_error) {
              ...
              var actionCitizenDetails = new CitizenModel.CitizenDetails(
                recDetails
              );
...
exports.updateReferAuth = [
  ...
  body("isUnrefer")
    ...
    .trim()
    .withMessage("nActive Status 0|1!")
    .isNumeric()
    .withMessage("isUnActive should be 0|1"),
          ...
          function (err, resDoc) {
            if (err) {
              ...
              if (resDoc) {
                CitizenModel.Citizen.findOneAndUpdate(
                  { citizenId: req.body.citizenId },
                  { $set: { isUnrefer: req.body.isUnrefer } },
                  function (ierr, iresDoc) {
                    if (ierr) {
        ...
        CitizenModel.Citizen.aggregate([
          ...
          if (user) {
            for (i = 0; i < users.length; i++) {
              ...
              if (
                temp.info.dateOfBirth != null &&
                temp.info.dateOfBirth != undefined &&
                temp.info.dateOfBirth != ""
              ) {
...
exports.citizenRefersAllReferAndNonPrescribed = [
  body("token").isLength({ min: 3 }).trim().withMessage("Invalid Token!"),
            ...
            $project: {
              ...
              cases: {
                $filter: {
                  input: "$cases",
                  as: "cases_field",
                  cond: { $and: [{ $eq: ["$$cases_field.status", 1] }] },
                },
              },
              "info.dateOfBirth": 1,
              "info.dateOfOnBoarding": 1,
      ...
      if (!errors.isEmpty()) {
        ...
        CitizenModel.Citizen.aggregate([
          ...
          if (user) {
            ...
            return apiResponse.successResponseWithData(
              res,
              "Prescribed Citizen List Fetch Successfully",
              users
            );
          } else
            return apiResponse.ErrorResponse(
              res,
              "Prescribed Citizen List Not Found"
            );
        });
      }
    } catch (err) {
      ...
      if (!errors.isEmpty()) {
        ...
        CitizenModel.Citizen.aggregate([
          ...
          if (user) {
            return apiResponse.successResponseWithData(
              res,
              "Citizen Cases List Fetch Fuccessfully",
              users
            );
          } else
            return apiResponse.successResponseWithData(
              res,
              "Citizen Cases not Found",
              []
            );
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        const allTasks = await CitizenModel.Citizen.find({
          citizenId: req.body.citizenId,
        });
        var condition = {};
        if (
...
exports.citizenListSearcherPagination = [
  ...
  async (req, res) => {
    ...
    // Find some documents
    await CitizenModel.Citizen.count(
            ...
            await CitizenModel.Citizen.aggregate([
              ...
              { $unwind: "$screener" },
              {
                $project: {
                  ...
                  "screener.lastName": 1,
                  screenerfullname: {
                    $concat: ["$screener.firstName", " ", "$screener.lastName"],
                  },
                  cases: 1,
                },
              },
            ...
            queryP = {
              $match: {
                $and: [
                  ...
                  {
                    lastName: { $regex: ".*" + vtemp[1] + ".*", $options: "i" },
                  },
                ],
              },
            };
        ...
        CitizenModel.Citizen.aggregate([
          {
            $match: {
              $or: [
                { citizenId: req.body.citizenId },
                { citizenLoginId: req.body.userId },
              ],
            },
          },
          { $limit: 100000 },
          ...
          if (user) {
            var cdate = "";
            user.info.dateOfOnBoarding = utility.toDDmmyy(
              user.info.dateOfOnBoarding
            );
            user.createdAt = utility.toDDmmyy(user.createdAt);

            if (
              user.info.dateOfBirth != null &&
              user.info.dateOfBirth != undefined &&
...
exports.updateCitizen = [
  body("citizenLoginId")
    .isLength({ min: 3 })
    .trim()
    .withMessage("Invalid CitizenLoginId!")
    .custom((value) => {
      return CitizenModel.Citizen.findOne({ citizenLoginId: value }).then(
        (user) => {
          if (user) {
          ...
          function (_error, newrecs) {
            if (_error) {
              ...
              UserDetailsModel.findOneAndUpdate(
                { userId: req.body.citizenLoginId },
                { $set: userRec },
                function (_derr, _dnewrecs) {
                  if (
...
exports.updateCitizenAddress = [
  ...
  body("country")
    ...
    .isAlpha()
    .withMessage("Country name must not contain number !"),
  body("state").isLength({ min: 2 }).trim().withMessage("Enter State !"),
  body("district").isLength({ min: 2 }).trim().withMessage("Enter District !"),
  body("address").isLength({ min: 3 }).trim().withMessage("Enter Address !"),
        ...
        CitizenModel.Citizen.aggregate([
          ...
          if (user) {
            for (var i = 0; i < users.length; i++) {
              ...
              users[i].info.dateOfBirth = utility.toDDmmyy(
                users[i].info.dateOfBirth
              );
            }
            return apiResponse.successResponseWithData(res, "Found", users);
          } else return apiResponse.ErrorResponse(res, "Not Found");
  ...
  (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        CitizenModel.Citizen.updateMany(
          { screenerId: req.body.screenerId },
