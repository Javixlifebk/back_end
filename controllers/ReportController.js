	  ...
	  try {
		...
		if (!errors.isEmpty()) {
		  ...
		  var html = fs.readFileSync(
			process.cwd() + "/helpers/templates/screening.html",
			"utf8"
		  );
		  var token = "hgaghsagf";
		  ...
		  var options = {
			...
			form: { caseId: caseId, ngoId: ngoId },
		  };
		  ...
		  var options1 = {
			...
			url: "http://3.111.147.53:3010/api/labtest/getHemoglobinList",
			headers: { "content-type": "application/x-www-form-urlencoded" },
			form: { caseId: caseId, token: token, ngoId: ngoId },
		  };
		  ...
		  var options11 = {
			...
			url: "http://3.111.147.53:3010/api/labtest/getHeartTestList",
			headers: { "content-type": "application/x-www-form-urlencoded" },
			form: { caseId: caseId, token: "adsdasdadasda", ngoId: ngoId },
		  };
			  ...
			  function (
				...
				if (status === 1) {
				  ...
				  if (
					...
					cdob =
					  users[0].citizendetails[0].dateOfBirth.split(
						"T"
					  )[0];
					var bdate = new Date(
					  cdob
					);
					users[0].citizendetails[0].dob =
					  bdate.getDate() +
					  "-" +
					  (bdate.getMonth() +
						1) +
				  ...
				  pdf
					...
					.then((val) => {
					  ...
					  (async () => {
						...
						var filename2 =
						  process.cwd() +
						  "\\uploads\\documents\\DISCLAIMER.pdf";
						merger.add(
						  filename2
						);
						var file =
