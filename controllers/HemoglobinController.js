...
exports.HemoglobinTestGreenList=[
	...
	//sanitizeBody("caseId").escape(),
	 ...
	 (req, res)=> {
	   ...
	   // Find some documents
	   HemoglobinModel.Hemoglobin.count({severity:0}, async (err, totalCount) => {
		 if (err) {
...
exports.HemoglobinTestAmberList=[
	...
	//sanitizeBody("caseId").escape(),
	 ...
	 (req, res)=> {
	   ...
	   // Find some documents
	   HemoglobinModel.Hemoglobin.count({severity:1}, async (err, totalCount) => {
		 if (err) {
		   response = { error: true, message: 'Error fetching data' }
		 }
		 HemoglobinModel.Hemoglobin.find({}, {}, query, async (err, data) => {
...
exports.HemoglobinTestRedList=[
	...
	//sanitizeBody("caseId").escape(),
	 ...
	 (req, res)=> {
	   ...
	   // Find some documents
	   HemoglobinModel.Hemoglobin.count({severity:2}, async (err, totalCount) => {
		 if (err) {
		   response = { error: true, message: 'Error fetching data' }
		 }
		 HemoglobinModel.Hemoglobin.find({}, {}, query, async (err, data) => {
